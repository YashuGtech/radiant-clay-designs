...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="flex items-center gap-2 px-1 font-display text-lg font-bold">
      <span className="size-3 rounded-md clay-mint" />
      {children}
    </h2>
  );
}
  ...
  return (
    <>
      ...
      <section className="clay space-y-4 px-6 py-6 text-center">
        ...
        <button
          onClick={onAddCard}
          className="clay-press w-full rounded-full clay-primary py-4 font-display text-base font-bold uppercase tracking-wide text-primary-foreground shadow-[0_12px_26px_-10px_color-mix(in_oklab,var(--primary)_70%,transparent)]"
        >
      ...
      <section className="clay space-y-4 px-6 py-6">
        ...
        {steps.map((s, i) => (
          <div key={s} className="flex items-start gap-3">
            <span className="clay-sm grid size-9 shrink-0 place-items-center rounded-full clay-mint font-display text-sm font-bold text-white">
              {i + 1}
            </span>
  ...
  return (
    <>
      ...
      <section className="clay space-y-4 px-6 py-6">
        ...
        <button
          onClick={onWithdraw}
          className="clay-press w-full rounded-full clay-mint py-4 font-display text-base font-bold uppercase tracking-wide text-white shadow-[0_12px_26px_-10px_color-mix(in_oklab,var(--mint)_75%,transparent)]"
        >
